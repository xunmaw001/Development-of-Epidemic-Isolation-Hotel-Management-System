<template>
    <div class="main-content">
        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '订单编号' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.fangjianOrderUuidNumber" placeholder="订单编号" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           ...
                           href="http://localhost:8080/gelijiudian/upload/fangjianOrderMuBan.xls"
                        >批量导入客房预定数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('fangjianOrder','导入导出')"
            ...
            <el-button v-if="isAuth('fangjianOrder','订单') && scope.row.fangjianOrderTypes == 101 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-sold-out" size="mini" @click="refund(scope.row.id)">取消预定</el-button>
            <el-button v-if="isAuth('fangjianOrder','订单') && scope.row.fangjianOrderTypes == 103 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-reading" size="mini" @click="openCommentModel(scope.row.id)">评价</el-button>
            <el-button v-if="isAuth('fangjianOrder','订单') && scope.row.fangjianOrderTypes == 101
    ...
    //当前表下拉框搜索条件
            ...
            json_fields: {
                //级联表字段
                     ...
                     '账户状态': 'jinyongTypes',
                //本表字段
                     '订单编号': "fangjianOrderUuidNumber",
                     '预定天数': "buyNumber",
                     '入住时间': "fangjianOrderTime",
                     '实付价格': "fangjianOrderTruePrice",
                     '订单类型': "fangjianOrderValue",
                     '支付类型': "fangjianOrderPaymentValue",
                     '订单创建时间': "insertTime",
            },
