    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(caipinOrder !=null){
            ...
            if(caipin != null){
            BeanUtils.copyProperties( caipin , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "yonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
...
//                            caipinOrderEntity.setCaipinOrderCanzhuoTypes(Integer.valueOf(data.get(0)));   //就餐餐桌 要改的
...
//                            caipinOrderEntity.setCreateTime(date);//时间
        ...
        CaipinOrderEntity caipinOrder = caipinOrderService.selectById(id);
            if(caipinOrder !=null){
                ...
                //级联表
                    CaipinEntity caipin = caipinService.selectById(caipinOrder.getCaipinId());
                if(caipin != null){
                    BeanUtils.copyProperties( caipin , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "yonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setCaipinId(caipin.getId());
                }
                //级联表
                    YonghuEntity yonghu = yonghuService.selectById(caipinOrder.getYonghuId());
    ...
    */
    ...
    public R add(@RequestBody CaipinOrderEntity caipinOrder, HttpServletRequest request){
        logger.debug("add方法:,,Controller:{},,caipinOrder:{}",this.getClass().getName(),caipinOrder.toString());
            ...
            caipinOrder.setCaipinOrderTypes(101); //设置订单状态为已预定饭菜
    ...
    public R add(@RequestParam Map<String, Object> params, HttpServletRequest request) throws ParseException {
        ...
        Integer userId = (Integer) request.getSession().getAttribute("userId");
            ...
            Integer caipinOrderCanzhuoTypes = Integer.valueOf(String.valueOf(params.get("caipinOrderCanzhuoTypes")));//就餐餐桌
            Integer caipinOrderPaymentTypes = Integer.valueOf(String.valueOf(params.get("caipinOrderPaymentTypes")));//支付类型
            String caipinOrderTime = String.valueOf(params.get("caipinOrderTime"));//预约时间
        ...
        for (Map<String, Object> map : caipins) {
           //取值
            ...
            caipinOrderEntity.setCaipinOrderCanzhuoTypes(caipinOrderCanzhuoTypes);//就餐餐桌 ？？？？？？
    ...
    */
    ...
    public R refund(Integer id, HttpServletRequest request){
        ...
        String role = String.valueOf(request.getSession().getAttribute("role"));
            ...
            caipinOrder.setCaipinOrderTypes(102);//设置订单状态为已取消预定
    ...
    */
    ...
    public R deliver(Integer id  , HttpServletRequest request){
        ...
        caipinOrderEntity.setCaipinOrderTypes(103);//设置订单状态为已接单
    ...
    public R receiving(Integer id , HttpServletRequest request){
        logger.debug("refund:,,Controller:{},,ids:{}",this.getClass().getName(),id.toString());
        CaipinOrderEntity  caipinOrderEntity = caipinOrderService.selectById(id);
        caipinOrderEntity.setCaipinOrderTypes(104);//设置订单状态为就餐
        caipinOrderService.updateById( caipinOrderEntity);
        return R.ok();
    }
